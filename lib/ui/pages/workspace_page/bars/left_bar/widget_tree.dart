...
class _WidgetTreeState extends State<WidgetTree> {
  ...
  bool isParentSelected(int index, int selectedPosition,
      List<IndexedObject<WidgetElement>> items) {
    ...
    if (selectedPosition >= items.length) return false;
    ...
    for (int i = selected.index! - 1; i >= 0; i--) {
      if (toTest.treePart![i] != selected.treePart![i]) return false;
    }
    return true;
  }

  ValueStream<ItemsAndSelected> get _theStream =>
      widgetBoard!.widgetElementsFlatAndCurrentlySelectedPosition.map((it) {
        return ItemsAndSelected(
            items: it.items,
            selectedPosition: it.items!
                .indexWhere((it2) => it2.object!.id == it.currentlySelected));
      }).shareValue();
        ...
        children: <Widget>[
          ...
          Expanded(
            child: StreamBuilder<ItemsAndSelected>(
                stream: _theStream,
                initialData: _theStream.valueOrNull,
                  ...
                  return SelectableList(
                    ...
                    itemBuilder: (context, index) {
                      return _WidgetItem(
                        ...
                        parentSelected:
                            isParentSelected(index, selectedPosition!, items),
                        onToggle: () {
                          AppScope.of(context)
                              .widgetBoard!
                              .toggleCollapsed(items[index].object!.id);
                        },
                        onTap: () {
                          AppScope.of(context)
                              .widgetBoard!
                              .setSelected(items[index].object!.id);
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Draggable(
        ...
        feedback: Container(
          padding:
              EdgeInsets.only(left: (element!.index! * 8).toDouble(), top: 4),
          child: Text(
            element!.object!.name,
        ...
        child: GlobalDragTarget<String>(
          ...
          onAccept: (childId) {
            ...
            AppScope.of(context)
                .widgetBoard!
                .acceptChild(element!.object!.id, childId, null);
          },
          builder: (context, accepted, rejected) {
            return InkWell(
              ...
              child: Container(
                ...
                color: selected || accepted.isNotEmpty
                    ? Colors.blue
                    : parentSelected!
                        ? Colors.blue.withOpacity(0.1)
                        : null,
                padding: EdgeInsets.only(
                    left: (element!.index! * 8).toDouble() +
                        8 /* Normal Padding */),
                child: Row(
                  ...
                  children: <Widget>[
                    GestureDetector(
                      onTap: onToggle,
                      child: element!.object!.hasChildSlots &&
                              element!.object!.children!
                                  .expand((it) => it.childrenIds!)
                                  .isNotEmpty
                          ? Icon(element!.object!.isCollapsed
                              ? Icons.arrow_drop_up
