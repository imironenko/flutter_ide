...
abstract class ICodeGenerator {
  String generateCode(Map<String?, CanvasElement> canvases,
      Map<String?, WidgetElement> allElements);
}
...
class SingleFileCodeGenerator extends ICodeGenerator {
  ...
  String generateCode(Map<String?, CanvasElement> canvases,
    ...
    List<Class> classes = canvases.keys.map((canvasId) {
      ...
      String? literalWidgetElementId =
          allElements[canvasId]!.children![0].childrenIds![0];
      LiteralWidgetElement? literalWidgetElement =
          allElements[literalWidgetElementId] as LiteralWidgetElement?;
      return Class((b) => b
        ..name = literalWidgetElement!.widgetName.value!.replaceAll(" ", "")
        ..extend = refer("StatelessWidget",
            "import 'package:widget_maker_2_0/material.dart'")
        ..methods.add(Method((a) => a
          ..name = "build"
          ..annotations.add(refer("override"))
          ..requiredParameters.add(Parameter((it) => it
    ...
    var library = Library((b) {
      ...
      b.directives.addAll([Directive.import("package:flutter/material.dart")]);
      // return b;
    });

    final l = DartFormatter().format('${library.accept(DartEmitter())}');
    print(l);
