...
typedef AcceptChild<T extends WidgetElement> = void Function(
    T element, String childId);
...
mixin ElementWidgetStateMixin<A extends WidgetElement?,
    T extends ElementWidgetMixin> on State<T> {
  late A? element;
  ...
  void initState() {
    ...
    isInReference =
        context.findAncestorWidgetOfExactType<InstancedWidgetElementWidget>() !=
            null;
  ...
  Widget? wrapWithDefault({Widget? child}) {
    ...
    return wrapWithSelector(child: wrapWithDraggable(child: child!));
  }
  ...
  Widget? getChildOrDragTarget(
      {String? childId, SlotData? data, WidgetWrapper? widgetWrapper}) {
    ...
    Widget result = DefaultDragTarget(
      id: widget.id,
      data: data,
    );
        ...
        builder: (context, snapshot) {
          ...
          return GlobalDragTarget<String>(
            onAccept: (childId) {
              AppScope.of(context)
                  .widgetBoard!
                  .acceptChild(this.id, childId, data);
            },
            onMove: onMove,
            builder: (context, accepted, rejected) {
              if (accepted.isNotEmpty) {
                return KeyedSubtree(
                  child: acceptingWidget,
                  key: ObjectKey(1),
                );
              } else {
                return KeyedSubtree(
...
class DefaultDragTarget extends StatelessWidget {
  const DefaultDragTarget({Key? key, required this.id, this.data})
      : super(key: key);
